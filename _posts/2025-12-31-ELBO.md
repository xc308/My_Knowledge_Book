...
\begin{aligned}
\mathcal{L}
&\ge - H_p(x^{(T)}) +  \int \mathrm{d}x^{(0:T)}\, q\!\left(x^{(0:T)} \right) \sum \sum_{t=2}^{T} \log \frac{p\!\left(x^{(t-1)} \mid x^{(t)}\right)}{q\!\left(x^{(t-1)} \mid x^{(t)},  x^{(0)} \right)} \times \frac{q\!\left(x^{(t-1)} \mid x^{(0)}\right)}{q\!\left(x^{(t)} \mid  x^{(0)} \right)}      \qquad (7)
\end{aligned}     
$$
