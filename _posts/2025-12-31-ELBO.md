...
The second term contains $p(x^{(t-1)} \mid x^{(t)})$ and $q(x^{(t)} \mid x^{(t-1)})$, which is hard to compare, so we leveraging $x^{(0)}$ and Bayesian trick to rewrite the $q(x^{(t)} \mid x^{(t-1)})$ to $q(x^{(t-1)} \mid x^{(t)})$ as 
...
\begin{aligned}
\mathcal{L}
&\ge - H_p(x^{(T)}) +  \int \mathrm{d}x^{(0:T)}\, q\!\left(x^{(0:T)} \right) \sum \sum_{t=2}^{T} \log \frac{p\!\left(x^{(t-1)} \mid x^{(t)}\right)}{q\!\left(x^{(t-1)} \mid x^{(t)},  x^{(0)} \right)} \times \frac{q\!\left(x^{(t-1)} \mid x^{(0)}\right)}{q\!\left(x^{(t)} \mid  x^{(0)} \right)} \\
&= - H_p(x^{(T)}) +  \int \mathrm{d}x^{(0:T)}\, q\!\left(x^{(0:T)} \right) \sum \sum_{t=2}^{T} \log \frac{p\!\left(x^{(t-1)} \mid x^{(t)}\right)}{q\!\left(x^{(t-1)} \mid x^{(t)},  x^{(0)} \right)} + \int \mathrm{d}x^{(0:T)}\, q\!\left(x^{(0:T)} \right) \sum \sum_{t=2}^{T} \log \frac{q\!\left(x^{(t-1)} \mid x^{(0)}\right)}{q\!\left(x^{(t)} \mid x^{(0)} \right)} \qquad (7)
\end{aligned}     
$$
